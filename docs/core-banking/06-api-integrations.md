...
| `POST /mandates/{id}/direct-debits` | `DIRECT_DEBIT_PRESENT` | montant, échéance, référence ; `Idempotency-Key` — exécuté tout de suite si l'échéance est arrivée (`COLLECTED`, `SETTLED` pour un créancier de la banque, ou `REJECTED` avec son motif, en `201`), en attente sinon ; refusé avant d'exister sur un mandat révoqué, hors validité ou au-delà de son plafond (`409`) |
