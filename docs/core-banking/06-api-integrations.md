...
| `POST /accounts/{id}/mandates`, `GET /accounts/{id}/mandates` | `MANDATE_REGISTER`, `DIRECT_DEBIT_READ` | référence, créancier (identifiant, nom ; compte de la banque ou banque et compte d'ailleurs), signature, validité, plafond par prélèvement — **202**, à deux dans l'agence du compte |
| `POST /mandates/{id}/revocation` | `MANDATE_REVOKE` | motif ; déjà révoqué, il le reste |
| `POST /mandates/{id}/direct-debits` | `DIRECT_DEBIT_PRESENT` (créancier d'ailleurs : la compensation), `DIRECT_DEBIT_ISSUE` (créancier de la banque : sa remise, plafonnée par rôle) | montant, échéance, référence ; `Idempotency-Key` — exécuté tout de suite si l'échéance est arrivée (`COLLECTED`, `SETTLED` pour un créancier de la banque, ou `REJECTED` avec son motif, en `201`), en attente sinon ; refusé avant d'exister sur un mandat révoqué, hors validité ou au-delà de son plafond (`409`) |
| `POST /accounts/{id}/issued-direct-debits` | `DIRECT_DEBIT_ISSUE` | montant, échéance, débiteur (nom, banque, compte), référence du mandat ; `Idempotency-Key` — crédité sauf bonne fin à l'échéance, bloqué jusqu'au règlement ; plafonné par rôle ; `409` si le produit n'a pas de compte d'encaissement |
| `POST /direct-debits/{id}/settlement`, `.../cancellation`, `.../refund`, `.../return` | `DIRECT_DEBIT_PROCESS` | règlement (compte nostro) ; retrait avant exécution ou rappel avant règlement (motif) ; remboursement d'un prélèvement reçu réglé (motif) ; retour d'un prélèvement émis (motif) — un état qui ne s'y prête pas est un `409` |
| `GET /direct-debits/{id}`, `GET /direct-debits?direction=&status=&page=&size=`, `GET /accounts/{id}/direct-debits` | `DIRECT_DEBIT_READ` | — ; lecture tracée |
| `POST /standing-orders` | `STANDING_ORDER_REGISTER` | compte, référence, nature (`FIXED` avec son montant, `SWEEP` avec son plancher), bénéficiaire (compte de la banque **ou** nom, banque et compte d'ailleurs), périodicité, date de début, fin ou nombre d'échéances, tentatives maximales, libellé — **202**, à deux dans l'agence du compte ; nature, périodicité et bénéficiaire validés à la soumission (`422`) |
| `POST /standing-orders/{id}/revocation` | `STANDING_ORDER_CANCEL` | motif, date ; ce qui est parti reste parti, rien de plus ne partira — un ordre déjà révoqué ou achevé est un `409` |
| `GET /standing-orders?status=`, `GET /standing-orders/{id}` | `STANDING_ORDER_READ` | la liste, ou l'ordre avec ce que chaque échéance a donné : exécution, rejet motivé, ou rien à balayer ; lecture tracée |
...
| `POST /regulatory/tax-rules` | `TAX_RULE_MANAGE` | code, libellé, assiette, taux de référence, **compte de collecte** (un compte général), validité — **202**, à deux ; taux hors bornes et compte client sont refusés à la soumission (`422`), un compte de collecte déjà pris par une autre taxe est un `409` |
| `GET /regulatory/tax-rules` | `REGULATORY_READ` | le catalogue des taxes de l'entité |
| `POST /regulatory/statement-packs` | `STATEMENT_PACK_MANAGE` | code, libellé, natures d'états citées, validité — **202**, à deux ; une liasse sans compte de résultat ne rapproche rien et se refuse à la soumission (`422`) |
| `GET /regulatory/statement-packs` | `REGULATORY_READ` | les liasses déclarées et leur composition |
| `POST /regulatory/consolidation-scopes` | `CONSOLIDATION_MANAGE` | code, libellé, devise de présentation, membres (entité, méthode, quote-part) — **202**, à deux ; une méthode inconnue se refuse à la soumission, un périmètre sans sa tête ou à quote-part partielle en intégration globale à l'exécution (`409`) |
| `POST /regulatory/consolidation-scopes/{id}/eliminations` | `CONSOLIDATION_MANAGE` | libellé et les deux comptes qui se font face, avec leurs entités — **202**, à deux ; un compte qui ne relève pas de l'entité annoncée est refusé |
| `GET /regulatory/consolidation-scopes` | `REGULATORY_READ` | les périmètres, leurs membres et leurs éliminations |
...
| `POST /fx-rates` | `FX_RATE_QUOTE` | devise, date, cours en unités de la devise de tenue, source — **202**, à deux ; un cours coté ne se réécrit pas (`409`), il se corrige par la cotation du jour suivant |
