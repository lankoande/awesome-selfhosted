...
| DC | Dossier client : pièces, politique, bénéficiaires effectifs, relations | ✅ V46 : `party_document` (pièce datée, émetteur, échéance, remplacée sans être effacée), `kyc_policy` / `kyc_policy_document` (pièces exigées par nature de tiers et par niveau, bénéficiaires effectifs requis, seuil de détention, à deux, remplaçable), `beneficial_owner` (personne physique, part, somme ≤ 100 %, déclaration et fin à deux) et `party_relationship` (représentant légal personne physique, détention sans cycle à quelque rang que ce soit, groupe lu en chaîne dans les deux sens) ; **complétude** confrontée à la politique — sans politique déclarée, rien n'est exigé — qui **restreint l'ouverture** en nommant ce qui manque, sans toucher aux comptes existants ; étape `DOCUMENT_EXPIRY` (non bloquante) qui constate une pièce expirée **une seule fois** et dont l'annulation de l'arrêté efface les constats ; quatre opérations au catalogue. Non fait : rescan périodique du portefeuille, matrice de restriction par opération | `PartyFileIT`, `TfjDocumentsIT`, `ApiIT.dossier_client` |
