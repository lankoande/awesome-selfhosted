...
| PL | Plafonds et paiements sortants | ✅ V40 : plafonds par opération, par jour et par mois du produit, plafond propre au compte posé à deux qui l'emporte, usage lu dans le journal hors contre-passations ; ordres de paiement sortants débités à l'ordre sur le compte de règlement, envoyés, réglés sur le nostro ou retournés (frais acquis), annulés avant envoi par contre-passation, rejeu par clé d'idempotence ; quatre opérations au catalogue. Non fait : chèques, prélèvements, cut-off, suspens remontés à l'arrêté | `LimitsIT`, `PaymentsIT`, `ApiIT.paiements_sortants` |
