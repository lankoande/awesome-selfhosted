...
| FX | Change : cours et positions | ✅ V45 : `fx_rate` (cours de référence par devise et par jour, source, à deux, jamais réécrit) et `fx_position` (compte de position et contre-valeur, sens imposé, comptes de résultat, marge tolérée) ; **le cours appliqué par une écriture est confronté à la référence** — devise sans position, cours absent ou marge dépassée sont des refus nommés —, hors contre-passation et reprise ; étapes `FX_RATES` (bloquante, avant tout calcul) et `FX_REVALUATION` (après les traitements comptables, avant le cliché) ; écart porté au résultat de change, annulation de l'arrêté qui le contre-passe ; trois opérations au catalogue. Non fait : cours acheteur et vendeur, positions par agence, position de liquidité, rapprochement `camt.053` | `FxIT`, `TfjFxIT`, `ApiIT.change` |
